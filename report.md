...
下面展示验证集中 4 条被代理 reward 判定为“含可见图元”的样本。左侧是数据集参考徽标，右侧是最终提交模型在 ModelScope 环境中重新生成并保存的真实输出；原始记录保存在 `examples/final_visible_4.jsonl`，图像未经过人工修复。由于最终 checkpoint-1 的全部 `lora_B` 为零，它与同口径 BF16 基座在参数层面等价，因此不能把右图描述为微调带来的提升。

![样本 0：儿童绘画徽标参考与最终输出](examples/final_comparison_00.png)

样本 0 虽然 XML 完整闭合，但实际输出只是多个白色矩形，且多数坐标超出 `viewBox`，视觉上接近空白。

![样本 5：心理关怀徽标参考与最终输出](examples/final_comparison_05.png)

样本 5 生成了圆、椭圆和多边形，但主要元素位于左上角或被后绘制的白色矩形遮盖，未表达提示词语义。

![样本 11：儿童泳池徽标参考与最终输出](examples/final_comparison_11.png)

样本 11 同样存在元素集中于原点、背景覆盖前景的问题，代理规则能检测到图元，却无法判断最终画面是否有效。

![样本 15：游戏工作室徽标参考与最终输出](examples/final_comparison_15.png)

样本 15 仍然只输出越界白色矩形。四个图像例子说明，“SVG 闭合”或“存在可见标签”只是最低语法代理，不等价于合格徽标，更不等价于提示词语义保真。
