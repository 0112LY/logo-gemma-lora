...
微调前的早期 BF16 基座评估与最终评估都使用 17 条验证样本和同一版 `reward.py`。基座评估采用早期推理脚本、最大生成长度 2048；最终评估使用合并后的 checkpoint-1、BF16、贪心解码、temperature 0、repetition penalty 1.0、最大生成长度 1024。最终逐样本结果记录在 `results.json`。

| 指标 | 微调前：BF16 基座 | 微调后：最终 checkpoint-1 | 表面变化 |
| --- | ---: | ---: | ---: |
| 样本数 | 17 | 17 | 0 |
| 平均代理 reward | 0.065882 | 0.290784 | +0.224902 |
...
| 人工渲染复核的可用徽标率 | 0%（0/17） | 0%（0/17） | 0 |

表中代理 reward 与格式指标出现表面提升，但不能归因于 LoRA：两次评估的生成长度和推理脚本不同，而且最终 checkpoint-1 的全部 `lora_B` 为零，LoRA 增量严格为零，在参数层面等价于 BF16 基座。人工渲染进一步发现，reward 标记为“可见”的 4 条最终输出均呈纯白画面，属于白色覆盖、元素越界等规则假阳性。因此本实验的可靠结论仍是：**未观察到微调带来的真实视觉提升，微调前后可用徽标率均为 0/17。**

作为反例，Exp7 checkpoint-5 已产生非零 LoRA 权重，但输出会在 path 中持续重复数字 `0`，达到长度上限仍不闭合；因此没有将该 checkpoint 作为最终提交模型。
