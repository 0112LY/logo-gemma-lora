...
![样本 0：儿童绘画徽标参考与最终输出](examples/final_comparison_00.svg)

样本 0 虽然 XML 完整闭合，但实际输出只是多个白色矩形，且多数坐标超出 `viewBox`，视觉上接近空白。

![样本 5：心理关怀徽标参考与最终输出](examples/final_comparison_05.svg)

样本 5 生成了圆、椭圆和多边形，但主要元素位于左上角或被后绘制的白色矩形遮盖，未表达提示词语义。

![样本 11：儿童泳池徽标参考与最终输出](examples/final_comparison_11.svg)

样本 11 同样存在元素集中于原点、背景覆盖前景的问题，代理规则能检测到图元，却无法判断最终画面是否有效。

![样本 15：游戏工作室徽标参考与最终输出](examples/final_comparison_15.svg)
