...
- 训练数据：过滤两条 prompt 为 `placeholder` 的记录后共 217 条
- 验证数据：17 条
...
该 reward 是结构和弱语义代理，不进行真实渲染，也不能充分判断构图和审美。

## 4. 实验过程

### 4.1 基座评估

微调前对 BF16 基座执行 17 条验证样本评估。最终对比统一使用 `student_kit/eval_self.py`，设置 seed 42、BF16、贪心解码、`num_beams=1`、`max_new_tokens=1024`、`repetition_penalty=1.0`，并在生成 `</svg>` 后停止。基座平均 reward 为 0.065882，XML 有效率为 11.76%，reward 判定的可见图形率为 0%。
...
| reward 中位数 | 0.060000 | 0.060000 | 0.000000 |
| XML 有效率 | 11.76%（2/17） | 11.76%（2/17） | 0 个百分点 |
| reward 判定的可见图形率 | 0%（0/17） | 0%（0/17） | 0 个百分点 |
...
本项目完成了数据检查、SVG reward、LoRA 超参数实验、checkpoint 选择和同口径的基座/微调后验证。最终各项 reward 差值均为 0，说明本次提交没有取得相对基座的提升；训练 loss 的下降也没有转化为自由生成质量提升。
