...
本项目使用 Gemma 3 270M 指令模型，根据自然语言提示生成可解析、可见且尽量符合提示的 SVG Logo。提交物包括 LoRA adapter、代理奖励实现、最终训练配置、验证结果和实验报告。

实验目标不是构建完整 SVG 渲染器，而是用可解释、可测试的结构指标判断输出是否具备基本可用性，并分析 LoRA 相对基座的行为。随机种子和数据种子均固定为 42。
...
`student_kit` 未出现在上游仓库是正常情况；本项目自行提供了 reward、评估和 PEFT 备用训练脚本。

## 3. 代理 Reward 设计

代理 reward 的权重为：有效性 30%、结构 15%、画布 15%、可见性 15%、简洁性 10%、提示词保真 10%、安全性 5%。主要检查包括：
...
该 reward 不计算贝塞尔曲线精确边界，不进行真实渲染，也不能充分衡量构图、审美和语义。因此下文只称“代理 reward”，不把分数提升解释为视觉质量提升。
...
下面给出三个可追溯样本。左列是数据集参考 SVG，中列是详细基座评估保存的真实输出，右列表示最终提交 adapter 在相同推理路径下的结果。由于最终 checkpoint-1 的全部 `lora_B` 为零，LoRA 增量严格为零，所以中、右两列在参数层面相同。图中的“XML 非法”和“空 SVG”是渲染状态说明，不是对模型输出的人工修复。

![示例 1：参考徽标、微调前和最终提交对比](examples/comparison_01.svg)

示例 1 的基座输出包含非法属性，XML 无法解析；最终零增量 adapter 没有改变这一结果。

![示例 8：参考徽标、微调前和最终提交对比](examples/comparison_08.svg)

示例 8 可以解析为 SVG，但没有任何绘图元素，因此画布为空；最终结果保持不变。

![示例 10：参考徽标、微调前和最终提交对比](examples/comparison_10.svg)

示例 10 同样是合法但无绘图元素的空 SVG。三个例子直观说明：本次最终提交优先保留了生成稳定性，但没有获得可证明的徽标质量提升。

作为反例，Exp7 checkpoint-5 已产生非零 LoRA 权重，但输出会在 path 中持续重复数字 `0`，达到长度上限仍不闭合；因此没有将该 checkpoint 作为“微调后”成功案例。
...
本项目完成了数据审计、可解释 SVG reward、BF16 LoRA、Q4 对照、checkpoint 级生成诊断和最终验证。实验没有取得可靠的 LoRA 提升，但通过基座对照、Q4 基座对照、零/非零 LoRA 权重检查和合并推理，定位了生成退化与训练 loss 脱节、量化路径异常和 adapter 运行时兼容问题。最终结果应被理解为一个被充分验证的“无提升”实验，而不是视觉质量改进。
